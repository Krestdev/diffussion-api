...
export class ServiceService {
  ...
  @AuditLog({ action: 'service.created', entityType: 'service', operation: 'CREATE' })
  ...
  async findOne(uuid: string) {
    const service = await this.database.service.findUnique({ where: { id: uuid } });
    if (!service) {
  ...
  @AuditLog({ action: 'service.updated', entityType: 'service', operation: 'UPDATE' })
  async update(uuid: string, dto: UpdateServiceDto) {
    return this.database.service.update({ where: { id: uuid }, data: dto });
  }

  @AuditLog({ action: 'service.deleted', entityType: 'service', operation: 'DELETE' })
  async remove(uuid: string) {
    try {
      await this.database.service.delete({ where: { id: uuid } });
    } catch (error) {
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2003') {
        throw new BadRequestException('Service is still assigned to one or more utilisateurs');
      }
      throw error;
    }
  }
